...
class IndexServer(SyncObj):
    def __init__(self, bind_addr, peer_addrs=None, conf=None, index_dir=None, logger=getLogger(NAME)):
        ...
        # open existing index on startup
    ...
    def delete_index(self, index_name):
        ...
        if index is not None:
            if index.storage.folder == '':
                ...
                self.__logger.info('deleting {0} in file storage on {1}'.format(index_name, index.storage.folder))

            pattern_toc = re.compile('^_{0}_(\\d+)\\.toc$'.format(index_name))
            for filename in index.storage:
                if re.match(pattern_toc, filename):
                    self.__logger.info(
                        'deleting {0}'.format(os.path.join(index.storage.folder, filename)))
